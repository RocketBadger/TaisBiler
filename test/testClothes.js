...
describe('Clothes', function () {
    it('create clothes empty', function () {
        ...
        should.equal(clothes.handedOut, {})
    })
...
describe('clothes statistics', function () {
    ...
    afterEach(async () => {
        ...
        await mongoose.connection.collections.clothess.drop()
    })
    it('get persons from a specific piece of clothes', async function () {
        ...
        await person2.save()
        await Clothes.addPerson(clothes._id.toString(), person._id.toString(), new Date())
        await Clothes.addPerson(clothes._id.toString(), person2._id.toString(), new Date())
