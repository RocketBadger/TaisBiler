...
describe('updateCar', () => {
    ...
    it('updateCar all attributes', async () => {
        let car = await Car.findOne({})
        car = await Car.updateCar(car, { brand: 'Mercedes', model: 'C1', licensePlate: 'BB22555', engine: 'BrumBrum', year: 2019, retired: false, colour: 'blue', id: 4 })
        car.brand.should.be.equal('Mercedes')
        car.model.should.be.equal('C1')
        car.licensePlate.should.be.equal('BB22555')
        car.engine.should.be.equal('BrumBrum')
...
describe('repair', () => {
    it('add repair today', async () => {
        ...
        await car.addRepair({ date: Date.now(), repair: 'Totalskadet', repaired: false })
...
describe('damage', () => {
    ...
    it('add two damages', async () => {
        const car = await Car.findOne({})
        await car.addDamage({ date: new Date(1995, 11, 24), damage: 'Ramt af slæde', repaired: true })
